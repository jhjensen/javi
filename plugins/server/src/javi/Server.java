...
final class Server implements Plugin, Runnable, EditContainer.FileStatusListener {
    ...
    */
   private static void serverCommand(String arg)  throws InputException {
      if (arg == null || arg.isEmpty() || "status".equals(arg)) {
         if (instance != null && !instance.stopped)
            throw new InputException("server running on port " + instance.port);
         else
            throw new InputException("server not running");
         ...
         try {
            ...
            throw new InputException("server: unknown arg: " + arg
               + " (use start, stop, status, or port number)");
         }
      }
   }

   public static Server startServer(int port) throws InputException {
      if (instance != null && !instance.stopped) {
      ...
      try {
         ...
         throw new InputException("server start failed: " + e.getMessage());
      }
   }
