...
//trace("vigroup doroutine rnum = " + rnum );
   switch (rnum) {
      case 1: cstyle(fvc); return null;
      case 2: cstylea(); return null;
      default:
          throw new RuntimeException("vigroup:default");
   }
}

static List<String>  listModFiles(String spec,FvContext fvc) throws InputException,IOException {
...
static void cstyle(FvContext fvc)  throws IOException{
    ...
    try {
        List<String> cmd = new LinkedList<String>(listModFiles(".*\\.java",fvc));
        cmd.add(0,"cstyle");
        cmd.add(0,"bash");
...
private void cstylea() throws IOException,InputException {
   ...
   if (dlist.length==0) 
      ...
      List<String> cmd = new LinkedList<String>(java.util.Arrays.asList(dlist));
      cmd.add(0,"./cstyle");
        cmd.add(0,"bash");
      PosListList.Cmd.setErrors(new CheckStyleInst(cmd));
   }
