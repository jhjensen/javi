...
abstract class Vt100 extends TextEdit<String> { //implements KeyEventDispatcher {
...
//   private Float oldsize;
   ...
   protected final Vt100Parser parser;

   Vt100(OutputStream ostri,BufferedInputStream istr,IoConverter<String> ioc) {
...
//        FontList.setFontSize(oldsize,currfvc.vi);
   ...
   void incX(int amount,StringBuffer sb) throws ReadOnlyException {
      insertString(sb);
      vtcursor.x += amount;
      if (vtcursor.x <0)
         vtcursor.x=0;
   ...
   void incY(int amount,StringBuffer sb)  throws ReadOnlyException{
      updateScreen(sb);
      vtcursor.y += amount;
      if (vtcursor.y <readIn()-rows)
         vtcursor.y=readIn()-rows;
   }
   void setX(int val,StringBuffer sb)  throws ReadOnlyException{
      insertString(sb);
      vtcursor.x=val-1;
   }

   void setY(int val,StringBuffer sb)  throws ReadOnlyException{
      //trace("setY " + val);
      updateScreen(sb);
   ...
   void setXY(int xval,int yval,StringBuffer sb)  throws ReadOnlyException{
   ...
   void eraseScreen(StringBuffer sb) throws ReadOnlyException {
      updateScreen(sb);
      int end = readIn();
   ...
   void eraseToEnd(StringBuffer sb) throws ReadOnlyException {
      updateScreen(sb);
   ...
   void eraseLine(StringBuffer sb) throws ReadOnlyException {
      insertString(sb);
      changeElementAt("",vtcursor.y);
   }

   void eraseChars(int count,StringBuffer sb) throws ReadOnlyException {
      insertString(sb);
      int delto = vtcursor.x+count;
      int strlen =  at(vtcursor.y).length();
      if (delto > strlen) {
         delto = strlen;
         trace("attempt to delete more character that possible delto "   + delto + " strlen " + strlen);
      }
      deletetext(false,vtcursor.x,vtcursor.y,delto ,vtcursor.y);
   }

   void insertLines(int count,StringBuffer sb) throws ReadOnlyException{
   ...
   void setInsertMode(boolean val,StringBuffer sb)  throws ReadOnlyException{
      insertString(sb);
      insertmode=val;
   }

   void updateScreen(StringBuffer sb) throws ReadOnlyException {
      insertString(sb);
      fixline();
   ...
   void saveCursor(StringBuffer sb)  throws ReadOnlyException{
      insertString(sb);
      savecursor.x = vtcursor.x;
      savecursor.y=readIn()-vtcursor.y;
   }

   void restoreCursor(StringBuffer sb)  throws ReadOnlyException{
      insertString(sb);
      vtcursor.y=readIn() - savecursor.y;
      vtcursor.x= savecursor.x;
   }

   private String fixline() throws ReadOnlyException{
   ...
   private void insertString(StringBuffer sb) throws ReadOnlyException {
        ...
        if (!insertmode && vtcursor.y<readIn()) while(sbused <text.length() ) {
           ...
           if (nindex ==-1) {
               ...
               if (vtcursor.x<=linelen)  {
                  ...
                  if (delval > linelen)
                     delval = linelen;
                  deletetext(false,vtcursor.x,vtcursor.y,delval, vtcursor.y);
               }
               
               vtcursor.set(inserttext (text.substring(sbused),vtcursor.x,vtcursor.y));
               sbused=text.length();
...
static class Telnet extends Vt100 {
  ...
  static String[] execstring1 ={"bash", "-i",""};
  static String[] execstring2 ={"ssh", "-t","-t",""};
...
static class CommReader extends Vt100 {
   ...
   CommReader(int baud,String comport,SerialPort porti) throws IOException{
      super(
