...
class TextList<TOType> extends TextEdit<TextEdit<TOType>> {
   ...
   TextList(IoConverter<TextEdit<TOType>> e,FileProperties<TextEdit<TOType>>fp) {
      super(e,fp);
   }
}
...
public final class PosListList extends TextList<Position> implements FileStatusListener {
   ...
   public boolean fileDisposed(EditContainer ev) { return false;/* don't care */}
   public void fileWritten(EditContainer ev) {/* don't care */}// ??? should undo positionlist fixes ??
   ...
   void addList(TextEdit<Position> poslist) throws ReadOnlyException {
...
public static class Cmd extends Rgroup {
   ...
   static private ArrayList<Position> tagstack = new ArrayList<Position>();
   private static PosListList inst;
   static {
      IoConverter io = new IoConverter(new FileProperties (
         FileDescriptor.InternalFd.make("position list list"),converter),true);
      inst = new PosListList(io);
   }
   private static final Matcher filereg = Pattern.compile(
       "(([a-zA-Z]:)?([^:\\s\\(\\)\"\']+)):([0-9]+)").matcher("");
   static private Ctag ctags;
   static private HashMap<String,TextEdit> tahash=new HashMap<String,TextEdit>();
     ...
     boolean dotmode) throws IOException,InputException{
      ...
      switch (rnum) {
         case 1: 
             ...
             return null;
         case 2: gototag(null,fvc); return null;
         case 3: poptag(fvc.vi); return null;
         case 4:  flush();  return null;
         case 5: 
            if (arg !=null) 
               inst.addList(DirList.getDefault().globalgrep(arg.toString())); 
            return null;
         ...
         case 12: UI.connectfv(DirList.getDefault(),fvc.vi); return null;
         case 13: UI.connectfv(TextEdit.root,fvc.vi); return null;
         default:
   ...
   public static void main (String args[]) {
      ...
      try {
         ...
         myassert(filereg.group(1).equals("smtp_hfilter.c"),filereg.group(1));
...
//      FileProperties fp = new FileProperties(FileDescriptor.InternalFd.make(sym),PositionIoc.converter);
   ...
   private static final Matcher classmatcher = Pattern.compile("\\bclass:(\\S*)\\b").matcher("");
   private static final Matcher filematcher = Pattern.compile("\\bfile:(\\S*)\\b").matcher("");
   ...
   private TextEdit taglookup(String str,View vi) throws InputException {
      ...
      try {
         ...
         if (tagcount>1) {
            ...
            out: for (int symindex = symlist.length-2;symindex>=0;symindex--) {
                ...
                for (int i=1;i<tagcount && findflag;i++) {
                   ...
                   if (filematcher.find() && filematcher.group(1).equals(currsym)) {
                      scores[i] += 1;
                      findflag = true;
                   }
...
private static class PllConverter extends ClassConverter<TextEdit<Position>> {

      public TextEdit<Position> fromString(String S) {
        PositionIoc ioc = new PositionIoc(S); // an unusable editvec
        return new TextEdit<Position>(ioc,ioc.prop); // an unusable editvec
      }
}
static PllConverter converter = new PllConverter();
}
