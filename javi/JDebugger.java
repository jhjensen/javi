...
void showthreads() {
  ...
  while ( titer.hasNext())  {
     ...
     try {
        ...
        e.printStackTrace();
         throw new RuntimeException("JDebugger.showthreads got unexpected exception");
     }
...
*/
transient private VirtualMachine vm;
transient private Vector<String> inarray = new Vector<String>();
...
JDebugger(String cname) throws IOException ,IllegalConnectorArgumentsException, VMStartException {
   super(new FileProperties(FileDescriptor.InternalFd.make("JDebugger" + cname),
      StringIoc.converter),true);
   LaunchingConnector con = com.sun.jdi.Bootstrap.virtualMachineManager().defaultConnector();
//   if (comd == null)
//      comd = new debugcom();
  
   Map<String,com.sun.jdi.connect.Connector.Argument> cargs =  con.defaultArguments();
...
//   cargs.get("options").setValue("-Xprof -Xshare:off -Dxxx=yyy");
   cargs.get("options").setValue("-Dsun.io.serialization.extendedDebugInfo=true");
