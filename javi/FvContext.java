...
private static class FvMap implements Serializable{
   ...
   private final class FvIterator implements Iterator<FvContext> {
      ...
      Iterator<HashMap<TextEdit,FvContext>>  viit = viewhash.values().iterator() ;
      Iterator<FvContext> fvit;
...
private boolean vis;
transient private UndoHistory.EhMark chmark;
transient private KeyHandler preDispatch;
...
static final Object getcurobj(TextEdit list) {
...
static String getCurrState() {
   StringBuilder sb = new StringBuilder( currfvc.fileposy + "," +  (currfvc.fileposx+1) );
...
public Object at() {
  ...
  if (fileposy<1)
    throw new RuntimeException("invalid fileposy " + fileposy);//cursoryabs(1);

  if (!edvec.contains(fileposy))
     throw new RuntimeException("invalid fileposy " + fileposy);//cursoryabs(edvec.finish()-1);
...
private static final int inrange(int val,int min,int max) {
   return val <min
      ? min
      : val >max
         ? max
...
public void changeElement(OType obj) throws ReadOnlyException {
   edvec.changeElementAt(obj,fileposy);
}
public void changeElementStr(String obj) throws ReadOnlyException {
   edvec.changeElementAtStr(obj,fileposy);
}

Position inserttext(String buffer) throws ReadOnlyException {
