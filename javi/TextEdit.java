...
static {
    StringIoc strio = new StringIoc("root EditContainer","should never see root container");
...
TextEdit(IoConverter<OType> e,OType[] inarr,EditContainer parent,FileProperties<OType> prop) {
  super(e,inarr,parent,prop);
}

TextEdit(IoConverter<OType> e,EditContainer parent, FileProperties<OType> prop) {
  super(e,parent,prop);
}
...
Position inserttext(String iStr,int xstart, int ystart) throws ReadOnlyException {
   ...
   if (ystart>=readIn()) {
      ...
      return new Position(sar.get(sar.size()-1).length(),readIn()-1,fdes(),"inserttext");
   }
   ...
   iStr = line.substring(0,xstart) + iStr + line.substring(xstart,line.length());
   ...
   return new Position(line.length()-xcursor,ystart,fdes(),"inserttext");
}

void substitute(int lineno,Matcher match_pattern, String subpattern,
       boolean globflag) throws ReadOnlyException {
   int i;
   char c;
   boolean slash = false;
   ...
   do {
      ...
      for (i=0;i<subpattern.length();i++) {
         ...
         if (slash){
           slash = false;
           if ((c>='0' && c<='9') ) 
               replace.append( match_pattern.group(c-0x30));
           else 
               replace.append(c);
         } else switch (c) {
            case '\\':
              ...
              break;
            case '&':
               replace.append(match_pattern.group(0));
               break;
            default:
               replace.append(c);
               break;
      ...
      //int matchstart = match_pattern.start()+replace.length();
      line = line.substring(0,match_pattern.start()) + replace + 
          line.substring(match_pattern.end(),line.length());
   } while (globflag);
   changeElementAtStr(line,lineno);
}

@SuppressWarnings("fallthrough")
public int processCommand(String com,int ypos)
   ...
   int linefinish;
   int lineto = 0;
   Matcher range_pattern = null;
   Matcher match_pattern = null;
   ...
   if (c =='g') {
      ...
      range_pattern=str.getpattern();
      //trace("pattern = " + range_pattern.pattern());
   }
   c = str.next();
   ...
   switch (c ) {
       case 's':
          c2 = str.peek();
          match_pattern=str.getpattern();
          //trace("match_pattern " + match_pattern);
       ...
       case 'w':
             ...
             if (str.peek()==0 && linestart == 1 && !containsNow(linefinish+1)) {
                 printout();
                 return ypos;
             }
             break;
   ...
   return commandproc2(linestart,linefinish,range_pattern, inverse,
      match_pattern,globflag,c,subpattern,
      lineto,str.remainder());
}

private int commandproc2(int linestart,int linefinish,Matcher range_pattern,
      boolean inverse, Matcher match_pattern,boolean globflag,char command,
      String subpattern, int lineto,String args
   ...
   switch (command) {
      case 'd':
         ...
         for (int i=linestart;i<=linefinish;) {
            if (range_pattern == null ||
                   (inverse ^ (searchForward(range_pattern,0,i))))  {
               delvec.add(remove(i,1).get(0)); // should change this to move elements and do one remove
               linefinish--;
            } else
               i++;
          }
      ...
      case 's':
         ...
         for (int i=linestart;i<=linefinish;i++)
            if (range_pattern == null ||
                   (inverse ^ (searchForward(range_pattern,0,i))))
             substitute(i,match_pattern,subpattern,globflag);
         checkpoint();
         return linestart;
   }
   ...
   if (!before && lineto <linefinish )
       throw new InputException("target within source moving to " + lineto + " from [" + linestart + "," + linefinish + "]");
...
String gettext(int xstart,int ystart,int xend,int yend) {
      return deletetext(true,xstart,ystart,xend,yend);
}

String deletetext(boolean preserve, int xstart,int ystart,int xend,int yend)  throws ReadOnlyException{
...
//  take care of firstline
    if (yend==ystart) {
       ...
       line = line.substring(0,xstart) + line.substring(xend,line.length());
       if (!preserve)
...
int shiftright(int start,int lineamount)
...
int shiftleft(int start,int lineamount) 
    throws ReadOnlyException  {
   //trace("amount = " + amount + " start = " +
   //  start + " lineamount= " + lineamount);
...
void changecase(int xstart,int ystart,int xend,int yend)  
     throws ReadOnlyException{
...
int joinlines(int count,int start) 
      throws ReadOnlyException { 
   int i;
   if (count==1)
      count=2;
...
void tabfix(int tabstop) throws InputException,ReadOnlyException {
   if (tabstop==0)
   ...
   for (int lineno = 1;lineno<finish();lineno++) {
      ...
      if (tabOffset!=-1) 
        changeElementAtStr(DeTabber.deTab(line,tabOffset,tabstop,new int[0]),lineno);
   }
   checkpoint();
}
...
static void perftest() throws IOException,InputException {
     ...
     long elapsed=0;
     {
        trace("start memory " + (Runtime.getRuntime().totalMemory()- Runtime.getRuntime().freeMemory()));
        Date start = new Date();
...
public static void main (String args[]) {
  UI.init(false);
  try { // forces some static initialization that would make debugging more confusing
  ...
  try {
     //FvContext.viewCount(); //simplfies debugging by moving static initialization here. circular dependency with editvec->undo->fvc
     test1();
     test2();
     test3();
     test4();
     ...
     Tools.doGC();
     trace("memory before clear " + (Runtime.getRuntime().totalMemory()- Runtime.getRuntime().freeMemory()));
     UI.flush();
     Buffers.clearmem();
     FvContext.dump();
     EditContainer.dumpStatic();
     Tools.doGC();
     trace("memory after clear " + (Runtime.getRuntime().totalMemory()- Runtime.getRuntime().freeMemory()));
