...
void addInsertBuf(InsertBuffer ins)  {
   insertbuf = ins;
}
...
transient protected InsertBuffer insertbuf; //??? circular dependency 
final boolean nextFlag;

protected enum Opcode {NOOP,INSERT, CHANGE,DELETE, REDRAW ,MSCREEN};
transient protected Opcode saveop;
transient protected int saveamount;

protected int screenposx;
transient protected static final int inset=2;
transient protected Graphics2D cursorg;

transient protected boolean delayerflag;

protected FvContext fcontext;
transient protected MarkInfo pmark = new MarkInfo();
...
transient private int savestart;
transient private boolean cursoron=false;
transient private boolean cursoractive=true;
transient private Color cursorcolor;
transient private Shape cursorshape;
...
void newfile(FvContext newfvc) {
  ...
  if (!fcontext.edvec.contains(1))
     throw new RuntimeException(fcontext.edvec + " must contain at least line one ");
...
private void ssetFont(Font font) {
   ...
   charwidth = (teststr.length()-1 + fontm.stringWidth(teststr))/teststr.length();
...
Graphics2D getGraphics2D() {
  Graphics2D g;
  if  ((g = (Graphics2D)super.getGraphics()) == null) {
        Tools.doGC();
