...
public class Command extends Rgroup {
...
public Object doroutine(int rnum,Object arg,int count,int rcount,FvContext fvc,
     boolean dotmode) throws InterruptedException{
   //trace("vic doroutine rnum = " + rnum);
   try  { switch (rnum) {
      case 0: return null; // noop
      case 1: return readFile(arg,fvc);
      case 2: fvc.vi.setTabStop(OToInt(arg));return null;
      case 3: Runtime.getRuntime().halt(0); return null;
      case 4: commandproc(fvc); return null;
      case 5: checkout((String)arg,fvc); return null;
      case 6: setcommand((String)arg,fvc); return null;
      case 7:  fvc.edvec.reload(); return null;
      default:
...
private Object readFile(Object arg,FvContext fvc) throws InputException,IOException{
   if (arg==null)
   ...
   if (fname.charAt(0)=='<') {
      ...
      fvc.edvec.insertStream(FileDescriptor.getBufferedReader(arg.toString()), fvc.inserty());
   return null;
}
...
private Object setcommand(String argline,FvContext fvc) 
...
static String getcomline(String prompt) {
  ...
  try {
     if (!(commFvc.at(commFvc.edvec.finish() -1).toString().equals(prompt))) {
        commFvc.edvec.insertOne( prompt, commFvc.edvec.finish());
     }
     commFvc.cursorabs(prompt.length(), commFvc.edvec.finish() -1);
