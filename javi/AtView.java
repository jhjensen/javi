...
class AtView implements AttributedCharacterIterator,Cloneable,java.io.Serializable {
...
void deTab(int tabstop) {
   ...
   if (tabOffset!=-1) {
       ...
       if (line2start!=Integer.MAX_VALUE)
          throw new RuntimeException("detabing and multiline display not supported");
       int [] tvals=new int[4];
       tvals[0]=highStart; //??? auto box for 1.5?
       tvals[1]=highFinish;
       tvals[2]=olineStart;
       tvals[3]=olineEnd;
...
static final Color  background= new Color(0,0,0);// 32 turns black (0,0,96); (0,0,64); (0,0,77);
 //new Color(0,0,50);// 32 turns black (0,0,96); (0,0,64); (0,0,77);
static final Color paraBackground = new Color(0,50,0);// 32 turns black (0,0,96); (0,0,64); (0,0,77);
...
static final Color unFinished = new Color(75,15,15); //(85,25,25);(50,10,10);
...
private static final Color lightBlue = new Color(0,0,128);// 32 turns black(0,0,150);
AtView(Font font) {
...
public Set<Attribute> getAllAttributeKeys() {throw new RuntimeException();}
...
private static final int leastgtpos(int val1,int val2,int pos)  {
...
public int getRunStart() {throw new RuntimeException();}
public int getRunStart(AttributedCharacterIterator.Attribute attribute) {throw new RuntimeException();}
