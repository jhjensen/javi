...
void bindCommands() {
     ...
     skeys.keyactionbind(KeyEvent.VK_F1,"nextpos",tt,InputEvent.SHIFT_MASK);
     ...
     mkeys.keyactionbind(KeyEvent.VK_LEFT,"backwardword",null, InputEvent.CTRL_MASK);
     ...
     mkeys.keyactionbind(KeyEvent.VK_RIGHT,"forwardword",null, InputEvent.CTRL_MASK);
     ...
     mkeys.keyactionbind(KeyEvent.VK_UP,"shiftmoveline",Boolean.FALSE ,InputEvent.SHIFT_MASK);
     mkeys.keyactionbind(KeyEvent.VK_UP,"movescreenline",mone ,InputEvent.CTRL_MASK);
     mkeys.keyactionbind(KeyEvent.VK_DOWN,"moveline",Boolean.TRUE,0);
     mkeys.keyactionbind(KeyEvent.VK_DOWN,"shiftmoveline",Boolean.TRUE ,InputEvent.SHIFT_MASK);
     mkeys.keyactionbind(KeyEvent.VK_DOWN,"movescreenline",one ,InputEvent.CTRL_MASK);
     mkeys.keyactionbind(KeyEvent.VK_END,"linepos",Integer.valueOf(Integer.MAX_VALUE),0);
     mkeys.keyactionbind(KeyEvent.VK_END,"gotoline",Integer.valueOf(Integer.MAX_VALUE),InputEvent.SHIFT_MASK);
     mkeys.keyactionbind(KeyEvent.VK_HOME,"linepos",zero,0);
     mkeys.keyactionbind(KeyEvent.VK_HOME,"gotoline",one,InputEvent.SHIFT_MASK);
     mkeys.keyactionbind(KeyEvent.VK_HOME,"gotoline",one,InputEvent.CTRL_MASK);
     mkeys.keyactionbind(KeyEvent.VK_END,"gotoline",null,InputEvent.CTRL_MASK);
     ...
     mkeys.keybind((char)10,"movelinestart",one,0); //??? major hack for enter=\n
     ...
     skeys.keyactionbind(KeyEvent.VK_F3,"regsearch",Boolean.FALSE,InputEvent.CTRL_MASK);
     ...
     skeys.keybind('^',"nextfile",null,InputEvent.CTRL_MASK|InputEvent.SHIFT_MASK);

     skeys.keybind(' ',"moveover",Boolean.TRUE,InputEvent.SHIFT_MASK);
     skeys.keybind(' ',"moveover",Boolean.FALSE,0);
     skeys.keybind('\036',"nextfile",null,InputEvent.CTRL_MASK|InputEvent.SHIFT_MASK);

      // editing keys
      skeys.keybind((char)2,"movescreen",mone,InputEvent.CTRL_MASK);
      skeys.keybind((char)18,"redo",null,InputEvent.CTRL_MASK); // ??? ALT should redo
      skeys.keybind((char)26,"redo",null,InputEvent.CTRL_MASK | InputEvent.SHIFT_MASK); // ??? ALT should redo
      skeys.keybind('Y',"redo",null,InputEvent.CTRL_MASK); // ??? ALT should redo
      skeys.keybind((char)8,"redo",null,InputEvent.SHIFT_MASK | InputEvent.ALT_MASK); // ??? ALT should redo
      ...
      skeys.keybind((char)8,"undo",null, InputEvent.ALT_MASK); // ??? ALT should redo
      skeys.keybind((char)26,"undo",null, InputEvent.CTRL_MASK); // ??? ALT should redo
      ...
      skeys.keyactionbind(KeyEvent.VK_DELETE,"deletetoend",null,InputEvent.SHIFT_MASK);
      skeys.keyactionbind(KeyEvent.VK_INSERT,"insert",ft,0);
}

boolean domovement(KeyEvent ein,int fiteratei,int riteratei, boolean dotmode,FvContext fvc) 
        throws InterruptedException,IOException,InputException {
   //trace("domovement fvc = " + fvc);
   KeyBinding binding;
   if ((binding= mkeys.get(ein)) !=null) {
       //trace("binding rg = " + binding.rg+ " event " + ein);
       binding.rg.doroutine(binding.index,binding.arg,fiteratei, riteratei, fvc,dotmode);
       return true;
   } else
       return false;
}

private boolean screenmovement(AWTEvent e1,FvContext fvc) throws InterruptedException,InputException,IOException {
   KeyBinding binding = skeys.get( (KeyEvent)e1);
   if (binding ==null)
      return false;
      //trace("binding  = " + binding);
   binding.rg.doroutine(binding.index,binding.arg,fiterate,riterate, fvc,false);
   return true;
        ...
        for (StackTraceElement elem:tr)  {
           if  (elem.getMethodName().indexOf("nextEvent") !=-1) 
              if  (elem.getClassName().indexOf("EventQueue") !=-1)  {
                 trace("caught while processing next event, probably not recoverable");
...
static abstract class JaviEvent {
  abstract void execute() throws IOException;
}

static private final int buttonmask  =   InputEvent.BUTTON3_MASK | InputEvent.BUTTON3_MASK
            | InputEvent.BUTTON3_MASK ;

void mousepress(MouseEvent event,FvContext fvc) throws InputException {
   ...
   switch (event.getButton()) {
      ...
      default:
         trace("no button ???? event modifiers = " +Integer.toHexString( event.getModifiers()));
   }
}
   ...
   if (event.getButton()  == MouseEvent.BUTTON1) {
      ...
      if (markpos!=null)
        if  (p.x==markpos.x && p.y == markpos.y)
          ...
          e.printStackTrace();
          throw new RuntimeException("mouserelease should not have gotten exception " +e ,e);
        }
   } 
}

boolean hevent(Object ev,FvContext fvc)  throws InputException, InterruptedException ,IOException{
     //trace("hevent" + awtEv);
  ...
  if (ev instanceof AWTEvent) {
     ...
     switch (awtEv.getID()) {
        ...
        case KeyEvent.KEY_PRESSED :
           ...
           if (((event.getKeyChar() != '0') || (aiterate != 0))
                 && (event.getKeyChar() >= '0' && event.getKeyChar() <= '9')) {
              aiterate = aiterate*10 + (event.getKeyChar() & 0x0f);
              return true;
        ...
        case ItemEvent.ITEM_STATE_CHANGED:
           ...
           String istr = ((CheckboxMenuItem)(ievent.getSource())).getActionCommand() +
