...
final class DeTabber {
   ...
   private final static void initts(int tabstop) {  
   ...
   static String deTab(String text ,int tabOffset,int tabstop,int tracking[]) {
      ...
      do {
         tbuf.replace(tabOffset,tabOffset+1,tabstrings[tabOffset%tabstop]);
   ...
   public static void main (String args[]) {
      try {
         //final static int tabFind(String line,int tabOffset,int tabstop,int charoffset) {
         // tab test
         Tools.Assert(tabFind("123456789012345678\t01234567\t901234567 ",18,8,34) == 27,null);
         Tools.Assert(tabFind("\tat ",0,8,8) == 1,null);
         Tools.Assert(tabFind("\tat ",0,8,10) == 3,null);
         Tools.Assert(tabFind("123456789012345678\t01234567\t901234567 ",18,8,0) == 0,null);
         Tools.Assert (tabFind("1234\t1234",4,4,5)==4,null) ;
         //trace ("" + tabFind("123456789012345678\t01234567\t901234567 ",18,8,37) );
         Tools.Assert(tabFind("123456789012345678\t01234567\t901234567 ",18,8,37) == 27,null);
         Tools.Assert(tabFind("\tat history.PersistantStack.set\tFile(PersistantStack.java:518",0,8,0) == 0,null);
