...
static class FileConverter extends ClassConverter<TextEdit<String>> {
   public TextEdit<String> fromString(String S) {
       //trace("S = "  + S);
       if (S.length() ==0) 
       ...
       FileDescriptor.LocalFile fh = FileDescriptor.LocalFile.make(S);

       if (EditContainer.findfile(fh)!=null)
          return findfileopen("dup file" + S + dummycount++, true);

       TextEdit<String> retval = findfileopen(fh, true);
       return retval==null
          ? findfileopen(S + "___bad filename" + dummycount++, true)
          : retval;
   }

   static FileConverter fileConverter = new FileConverter();
   static private int dummycount;
   static private TextEdit<String> findfileopen(FileDescriptor.LocalFile fh, boolean createok) {
        //trace("fh = " +fh);
   ...
   static private TextEdit<String> findfileopen(String filename, boolean createok) {
...
private class Commands extends Rgroup {
   ...
   public Object doroutine(int rnum,Object arg,int count,int rcount,FvContext fvc,
        boolean dotmode) throws InputException,IOException,MapEvent.ExitException {
       //trace("rnum = " + rnum);
       switch (rnum) {
         ...
         case 3: processZ(fvc); return null;
         case 4: nextfile(fvc); return null;
         case 5: UI.connectfv(instance,fvc.vi); return null;
         case 6: 
         case 7:quit(true,fvc); return null;
         case 8: 
  ...
  FileListEvent(List lst) {
   ...
   void execute() {
     try {
        ...
        while (eve.hasNext()) 
           Rgroup.doroutine("vi",eve.next().toString(),1,1,FvContext.getCurrFvc(), false);
...
static ArrayList<EditContainer> writeModifiedFiles(String spec) throws IOException,InputException{
   return instance.writeModifiedFilesi(spec);
}

private ArrayList<EditContainer> writeModifiedFilesi(String spec) throws IOException{
...
static boolean gotoposition(Position p,boolean setstatus,View vi ) throws InputException {
...
private boolean openFile(Position pos,View vi) 
   ...
   if (ec==null && fh instanceof FileDescriptor.LocalFile) {
         ...
         if (fhl.canRead() && fhl.isFile()) {
            ...
            TextEdit<String>text = new TextEdit<String> (new FileInput(fp),instance,fp); //??? should be filelist
            if (!fh.canWrite())
...
private static class FileParser extends BufInIoc<TextEdit<String>> {
   ...
   //   super(new FileDescriptor(FileDescriptor.Types.Internal,"filelist"));
   //   threadstart=true;
      super(fp,true);
       ...
       case 1:  //trace("stage 1 ");
          ...
          if (FileDescriptor.isSpecial(searchName))  {
             ...
             if (fh instanceof FileDescriptor.LocalDir) {
                foundf |= DirList.getDefault().addSearchDir((FileDescriptor.LocalDir)fh);
                continue oloop;
             } else {
                if (EditContainer.findfile(fh)!=null)
                   dupflag=true;
                else if (fh instanceof FileDescriptor.LocalFile){
                   edv = FileConverter.findfileopen((FileDescriptor.LocalFile)fh,true);
                   if (edv!=null)
                      break oloop;
                }
             }
          ...
          stage=2;
          // nobreak
       case 2: //trace("stage 2 looking for " + searchName + "in directory list");
           DirList dlist = DirList.getDefault();
           while(true) {
        ...
        case 3: //trace("stage 3 regexp directory list search:" + searchName + " edv = " + edv);
              ...
              if (fh instanceof FileDescriptor.LocalDir ) {
                ...
                if (EditContainer.findfile(fh)!=null)
                   ...
                   if (EditContainer.findfile(fh)!=null)
                      ...
                      edv = FileConverter.findfileopen((FileDescriptor.LocalFile)fh,false);
                      if (edv !=null) 
                          break oloop;
                    }
                 }
        ...
        case 4: //trace("stage 4  if all else fails create file" + searchName + " edv = " + edv);
           //trace("foundf = " + foundf);
           if (!foundf) {
...
private static void processZ(FvContext fvc) throws MapEvent.ExitException,InputException {
   //trace("processZ");
   ...
   if (EventQueue.nextKey(currview) == 'Z') {
      try {
         ...
         if (ev.parentEq (instance) ||
            // gross hack because if a dummy file is created first the parent is root.
              (ev.parentEq(TextEdit.root) && (-1 != instance.indexOf(ev)))) {
            if (ev.isModified()) 
               ev.printout();
         ...
         UI.reportMessage("unable to save file" +e);
          ...
          TextEdit newfile = (TextEdit)FvContext.getcontext(currview,instance).at();
...
//     trace("instance[3] " + instance.at(3));
     Tools.Assert((instance.at(1)).finish()==2,Integer.valueOf((instance.at(1)).finish()));
     Tools.Assert((instance.at(2)).finish()==2,Integer.valueOf((instance.at(1)).finish()));
     instance.remove(1,1);
     instance.checkpoint();
     //trace("instance[1] " + instance.at(1));
