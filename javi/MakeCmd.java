...
public Object doroutine(int rnum,Object arg,int count,int rcount,FvContext fvc,
   ...
   switch (rnum) {
      case 1: cccommand(fvc); return null;
      case 2: mkcommand(fvc); return null;
...
void cccommand(FvContext fvc)  throws IOException{
    ...
    try {
       ArrayList<EditContainer> efs = FileList.writeModifiedFiles(".*\\.((cpp)|[hc]|(as))");
       String files2 = l2String(efs,fvc.edvec.getName());
       PosListList.Cmd.setErrors(new GccInst("",
            "d:\\cygwin\\bin\\bash -c  \"gcc -Wall -S  -DUSESPINE -DETI_ADDED -DSTANDARD_OVERHEAD -DSTANDARD_HYBRID -DSTANDARD_CASCADE -DIBM_VERSION -DNDEBUG -DPURE_INTEGER -DSTANDARD_INTEGER -DDELTA_STACK_SPACE_OPT -DC_CODE -DSTAND -DSTRIP=1 -DEMBED  -I. -Ie:/build/tools/tornado/mips/v2.0.1/target/config/all -Ie:/build/tools/tornado/mips/v2.0.1/target/h -Ie:/build/tools/tornado/mips/v2.0.1/target/src/config -Ie:/build/tools/tornado/mips/v2.0.1/target/src/drv -DCPU=VR4100 -DMIPSEL -DSOFT_FLOAT  -DIBM_EMBEDDED -DIBM_EMBEDDED_RAL -DIBM_EMBEDDED_EAL -DIBM_EMBEDDED_NO_INI_FILE  -DNATIVE_ONLY -DNO_IOSTREAM -DSUBSYS_TTS -DIBM_EMBEDDED_NO_ETI_TRACE -DTGTOS_VXWORKS -DBOARD_HSPS7550 -DSUBSYS_ -DBLDTYPE_DEBUG  -DLINK_STATIC_ECI -DIBM_WIN_EMBEDDED -DNO_REALWORLD_SUPPORT -DETISYS_TYPE_VXWORKS -DREENTRANT_KLATT -DDYNAHEAP -DCONVERGED -DECI_LITTLE_ENDIAN    -DNO_FILE_IO -DLINK_STATIC_ENGINE_FRA -DLINK_STATIC_ENGINE -DfirstDate=\\\"1999\\\" -I. -Iexports/DEBUG/inc/ -Isrc/apps/speakapp"
            + files2 + "\" ",false));
...
void mkcommand(FvContext fvc)  throws IOException{
    ...
    try {
       FileList.writeModifiedFiles(".*");   // write out all files ??? give better name
...
//       String[] cmd = {"c:\\cygwin\\bin\\perl","make.pl" , files};
       ...
       PosListList.Cmd.setErrors(new PositionIoc("mk " + files,Tools.runcmd (cmd)));
...
void asmcommand(FvContext fvc)  throws IOException {
    ...
    try {
       ArrayList<EditContainer> efs = FileList.writeModifiedFiles(".*\\.((as)|(asm))");
       PosListList.Cmd.setErrors(new GccInst(l2String(efs, fvc.edvec.getName()),
             "c:\\v8\\v8asm.exe ",true));
   } catch (InputException e) {
      throw new RuntimeException("cccommand has bad spec",e);
   }
...
GccInst(String filesi,String comstringi,boolean asmflagi)
     throws IOException {
