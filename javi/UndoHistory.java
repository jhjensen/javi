...
class UndoHistory<OType> extends PersistantStack {
   ...
   void baseRecord() {
       ...
       if (currmark.hasNext())
          ...
          currmark.push(current = new ChangeRecord.BaseRecord(currmark.getIndex() +1)) ;
   ...
   int undo() throws IOException {
      ...
      do {
         ...
         //trace(" " +currmark.getIndex() + " " + current);
      } while (!((current = (ChangeRecord)currmark.previous()) instanceof ChangeRecord.CheckRecord));
      //trace(" " +currmark.getIndex() + " " + current);
      return chindex;
   }
   ...
   static class BackupStatus {
     ...
     public String toString() {
         return "clean quit  = " + cleanQuit + "  isQuitAtEnd = " + isQuitAtEnd + 
            " + error = " + error;
     } 
   ...
   BackupStatus readToQuit() throws IOException {
      ...
      //trace("dump= " + dump());
      BackupStatus retval = new BackupStatus (cleanClose(), isQuitAtEnd(),null);
      //trace("status = " + retval);
      return retval;
   }
   ...
   */
   ...
   class EhMark<OType> extends PersistantStack.PSIterator implements Cloneable {
      ...
      public boolean matches(Object ob,Object ob2) {
         ...
         return (ch.getType() == ch2.getType()) && ch.getType() == ChangeRecord.BASE;
      }
   ...
   static abstract class ChangeRecord<OType> {
      ...
      final static int INSERTSTRING = 7;
      int cindex;
      ...
      private static class WriteRecord extends UndoHistory.ChangeRecord.CheckRecord {
         String writeDate;   
         public String toString() {
         ...
         void writeExternal(DataOutputStream dos,ClassConverter conv) {
             ...
             dos.writeUTF(writeDate);
                ...
                throw new RuntimeException("ehsistory.WriteRecord.writeExternal failed",e);
             }
         }
         WriteRecord() {}
