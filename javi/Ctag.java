...
private static final class TagEntry {
   ...
   Position [] getPositions(String tagfile) throws IOException {
       if (positions==null) {
          ...
          do {
           ...
           } while (fileend> ctfile.getFilePointer()); // where to start again backwards
   ...
   public String toString() {
      return name + " " + ( positions == null ? -1 : positions.length) + " " +  filestart  + " " + fileend + " " ;
   }
...
Ctag(String tagfilename) throws IOException{
   ...
   carray.add(new TagEntry( "" + (Character.MAX_VALUE), ctfile.length(),ctfile.length()));
   ctfile.close();
}
...
private TagEntry filefind(RandomAccessFile ctfile, long start,long end) throws IOException{
     ...
     if ((ctfile.getFilePointer()> end) || (line == null)) { // must have come to end of file
        ctfile.seek(start);
     ...
     return backmark==endmark
        ? null
        : new TagEntry(curtag.substring(0,curtag.length()-1),backmark, endmark);
}
